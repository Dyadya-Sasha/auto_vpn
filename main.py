...
def ssh_connect():
    try:
        ...
        _stdin1, _stdout1, _stderr1 = client1.exec_command(
            'sed -i 2d /etc/hosts; sed -i \'2i127.0.1.1     {}  combined-ipdr01\''.format(hostname) + ' /etc/hosts;' + 'sed -i 3d /etc/hosts; sed -i \'3i{}\' /etc/hosts;'.format(puppet_repo) + 'echo {}'.format(hostname_short) + ' > /etc/hostname; '
                                                                                                                                                                               'hostname -F '
                                                                                                                                                                               '/etc/hostname; '
                                                                                                                                                                               'hostname -f')
        print("\n----------------")
...
def puppet_prepare(segment, provider_name, city):
    ...
    puppet_cmd = "cat /home/$USER/puppet_skel > {}/s8/prod/{}/{}/{}/sorm-2-3/combined-ipdr/nodes/combined-ipdr01.sorm-2-3.{}.{}.{}.prod.s8.norsi-trans.org.yaml".format(PUPPET_GIT, city, provider_name, segment, segment, provider_name, city)
    subprocess.run(puppet_cmd, check=True, shell=True)
...
if __name__ == '__main__':
    ...
    try:
        ...
        Puppet_push = ("cd /home/$USER/{}; git add --all; git commit -m \"Added iptables for client '{}'\"; git push ".format(PUPPET_GIT, sys.argv[3]))
        print("\nPUSHING Pupper CMD {}".format(Puppet_push))
        ...
        print(exit_status)
        if exit_status == 2:
            print("Puppet agent on Puppet_git did its work successfully")
        else:
